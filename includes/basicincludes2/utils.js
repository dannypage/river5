...
function jsonStringify (jstruct, flFixBreakage) { //7/30/14 by DW
	//Changes
		//6/16/15; 10:43:25 AM by DW
			...
			//https://river4.smallpict.com/2015/06/16/jsonEncodingIssueSolved.html
...
function readHttpFileThruProxy (url, type, callback) { //10/25/14 by DW
	var urlReadFileApi = "https://pub2.fargo.io/httpReadUrl"; //"https://pub2.fargo.io:5347/httpReadUrl";
	if (type === undefined) {
		type = "text/plain";
		}
...
function getDomainFromUrl (url) { //7/11/15 by DW
	if ((url != null ) && (url != "")) {
		...
		var root = url.split('?')[0]; // cleans urls of form https://domain.com?a=1&b=2
		url = root.split('/')[2];
		}
	return (url);
	};
...
function hitCounter (counterGroup, counterServer, thisPageUrl, referrer) { //8/12/14 by DW
	var defaultCounterGroup = "scripting", defaultCounterServer = "https://counter2.fargo.io:5337/counter";
